...
async function main() {
    ...
    for (let disptach of dispatches)
        post.post('se.madfrenzy.com', 'seworld.' + disptach, async products => {
                    ...
                    if ('main_pair' in product.pairs)
                        for (let size in product.pairs.main_pair)
                            if (!isNaN(Number(size)))
                                console.log(image_converter.convert(product.pairs.main_pair[size].absolute_path))

                    if ('pairs' in product.pairs)
                        for (let id in product.pairs.pairs)
                            for (let size in product.pairs.pairs[id])
                                if (!isNaN(Number(size)))
                                    console.log(image_converter.convert(product.pairs.pairs[id][size].absolute_path))
                }

                if ('details_main_pair' in product)
                    if ('detailed' in product.details_main_pair)
                        console.log(image_converter.convert(product.details_main_pair['detailed'].absolute_path))
            }
            
            await db.set(disptach, JSON.stringify(products))
        })
}
